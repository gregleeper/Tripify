...
class TripsDriver extends Component {
  state = {
    pageSize: 5,
    currentPage: 1,
    selectedDate: null,
  ...
  columns = [
    {
      ...
      label: "Trip Title",
      content: trip => (
        <Link to={`/trips/driver/view/${trip._id}`}>{trip.title}</Link>
      )
    },
    ...
    { path: "departureLocation", label: "Departing From" },
    { path: "tripOwner.name", label: "Created By" },
    {
      path: "returnTime",
      label: "Returning At",
      content: trip => (
        <Moment local="en" format="ddd MMM-DD-YYYY h:mm A">
          {trip.returnTime}
        </Moment>
      )
    }
...
const mapStateToProps = state => {
  return { trips: Object.values(state.driverTrips) };
};
